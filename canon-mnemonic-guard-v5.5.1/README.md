# 三省引擎 (CMG) — Canon-Mnemonic-Guard v5.5.1
...
安装 [skill-autoload](https://github.com/L1veSong/skill-autoload) 插件，CMG 在每次会话自动生效，无需手动操作：
...
> 需要 Hermes 包含 `pre_system_prompt` 钩子（已提交 PR，等待合入）。
> 钩子合入前，可用本地 patch 或 SOUL 一行激活作为过渡。
