...
_comment: "v5.2.1 SOUL激活: Canon v2.5.2 + Guard v4.5.0 + Mnemonic v3.3.0 + CMG外观。init时询问是否在SOUL.md写激活标记，扫盘自动检测标记状态。"
...
| 推荐 | 类型 | 增强点 | 集成方式 |
|------|------|--------|---------|
| `rtk-hermes` | Hermes 插件 | 压缩终端输出 60-90% token | 被动受益。rtk 压缩所有 Hermes 输出 → CMG 的 pre_action 检查自然消耗更少 token。rtk 的功能范围不受 CMG 影响。安装指南: `references/rtk-hermes-setup.md` |
...
| 推荐 | 类型 | 增强点 | 集成方式 |
|------|------|--------|---------|
| `hermes-agent-skill-authoring` | Skill | 发布流程风控 | 13 项发布自检清单 + 版本号全文件 grep 验证。各司其职——典忆卫管对话行为，authoring 管发布流程 |
