...
| 推荐 | 增强点 | 集成方式 |
|------|--------|---------|
| `skill-autoload` | 自动加载 CMG，零手动 | Plugin，`pre_system_prompt` 钩子 |
| `cmg-guard` | 硬拦截 AI 输出，不可绕过 | Plugin，`transform_llm_output` 钩子 |
...
### 坑点 13: 激活状态数字必须验证后报告（2026-05-25）

激活消息 "三省引擎已激活。X条禁止/Y条缺失/Z条偷懒" 中的数字，必须 count rules/目录实际文件数。禁止凭记忆或感觉报数。

**实战：** 将 33/8/9 错报为 10/2/2，用户当场纠正「记住不能瞎说！」。

**正确做法：** 输出激活消息前，search_files rules/ban/ rules/gap/ rules/lazy/ 分别 count。不能省这一步。
