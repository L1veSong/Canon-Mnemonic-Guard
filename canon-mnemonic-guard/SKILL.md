...
description: 三省引擎 (CMG) — 取自「吾日三省吾身」。v5.5.4 cmg-guard v1.2.0步骤完整性检查+分阶段升级。对外 role:guard stage:pre_action，内部三模块各自独立。
version: 5.5.4
role: guard
dependencies: [canon, guard, mnemonic]
_comment: "v5.5.4 cmg-guard v1.2.0(+step-check+escalation+post_llm_call)。Canon v2.7.2 + Guard v4.8.2 + Mnemonic v3.5.3。skill-autoload v1.0.1 + cmg-guard v1.2.0。"
...
| 版本 | 变更 |
|------|------|
| v5.5.4 | +cmg-guard v1.2.0：pre_llm_call步骤完整性检查+分阶段升级系统+post_llm_call钩子 | 当前 |
...
**必须输出**: "三省引擎 v5.5.4 已激活。X 条禁止 / Y 条缺失 / Z 条偷懒。典则·护栏·忆存。"
...
- [v5.5.4 发布教训](references/v5.5.4-release-lessons.md) — 发布流程六条铁律（不动未变更包/README增量/CHANGELOG保留/发布前diff/零隐私/init.py盲区）
...
| `cmg-guard` | v1.2.0: 硬拦截 + 步骤完整性检查 + 分阶段升级 | Plugin，`pre_llm_call` + `transform_llm_output` + `post_llm_call` 三钩子。配套插件，非核心四线。详见 `references/cmg-guard-v1.2.0-escalation.md` |
...
**症状：** 创建 persistent 文件（planning files）后声称"记完了"，但遗漏了本会话多个讨论点。CMG Guard 五层拦截器全部放行——因为"内容不完整"不属于 ban 关键词、非 fabrication、非跳步骤。

**根因：** CMG 拦截器管行为违规（说了禁词/编造/跳步骤/不加载skill/不确认），不管语义层面的内容完整性。

**缓解：** rule_060 (ban/hard) 已新建——创建持久化文件后强制逐条核对覆盖度。但这条规则依赖 AI 自觉遵守，cmg-guard 插件层无法自动检测"内容遗漏"。根本解决需要 B 层 LLM 语义判断（双层防御模型规划中）。
