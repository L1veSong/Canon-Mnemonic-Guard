---
name: mnemonic
description: 三省引擎(CMG)忆存线 (Mnemonic) — 状态记忆层。v3.5.2 补全M3(!patterns+!datasource)。v3.5.1 补全P2 session追踪。纯记忆状态层，不生产规则不执行拦截。
version: 3.5.2
role: memory
stage: background
dependencies: []
...
>
> v3.5.2 补全 M3：!patterns（查看识别模式）+ !datasource（数据源状态）。v3.5.1 P2 session追踪。读取 Guard 的 intercept_log.jsonl 和 state.json，自动识别高频错误模式，推送规则草稿至 Canon 固化引擎。不生产规则、不执行拦截。
...
**必须输出**: "Mnemonic v3.5.2 已激活。数据源: {guard_intercept/canon_errors/none}。近7天拦截{N}次。P2加速就绪(同会话2次→推送)。上下文保留: 就绪。模式识别: {on/degraded/off}。"
...
> Guard P1 已实现「同会话第2次命中→block」。Mnemonic v3.5.2 消费 Guard 拦截日志，在同会话累计第2次命中时立即推送草稿——不再等待历史积累。
