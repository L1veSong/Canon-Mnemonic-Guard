...
### 坑点 8: 输出层拦网无法拦截执行层偷懒（架构级边界）

**症状：** Agent 加载了 skill 文档但没有执行完整 workflow——声称完成了，但中间步骤被跳过。五层拦截器全部未触发。

**根因：** 拦截器只能检查输出文本（关键词匹配），无法感知 Agent 在执行层是否跳过了中间步骤。这是架构级边界，不是 bug。

**防御：**
- 短词关键词无法覆盖语义级违规——"加载了文档但没执行workflow"不是词表能匹配的
- cmg-guard pre_llm_call 的步骤完整性检查可以在 LLM 调用前阻断（v1.2.0+）——这是当前唯一的系统级防御
- 最终防线：用户直接纠正 → errors.jsonl → 固化规则 → 下次会话 Guard 提醒。但 Guard 不能替 Agent 执行。

### 坑点 8: 输出层拦截不覆盖执行层跳步骤

五层拦截器在 Skill 层运行，仅检查输出关键词。Agent 跳过中间步骤不产生违规关键词→拦截器不触发。这是架构边界，非 bug。防御靠用户纠正→固化规则，或 cmg-guard pre_llm_call 步骤检查。
