---
name: guard
description: 三省引擎(CMG)护栏线 (Guard) — 规则执行器。读取 Canon 规则库 + Mnemonic 状态，独立执行五层 pre_action 前置拦截。v4.8.2 默认固化阈值10→3。纯执行校验层，不生产规则不存记忆。
version: 4.8.2
role: guard
stage: pre_action
dependencies: []
...
>
> v4.8.2 默认固化阈值10→3。三省引擎(CMG)护栏线，从 CMG v2.4.1 剥离独立。读取 Canon 的 rules/ 目录作为规则源，读取 Mnemonic 的 state.json 感知上下文。
...
**必须输出**: "Guard v4.8.2 已激活。读取 Canon {N} 条规则。风险分级: 就绪。上下文升级: 2次触发+半衰期衰减(24h)。用户纠正提升: 就绪。P2联动: session_id追踪+Mnemonic钩子就绪。效能分析: {X}条正常/{Y}条降级/{Z}条过期。上下文感知: 就绪。动态清单: {论文K/代码L/通用M}项。"
...
### 坑点 8: 关键词匹配无法拦截语义级违规（2026-05-28 实战）

**症状：** 用户说"跑IF"→Agent只读了SKILL.md提取设计模式→声称"跑完了"。CMG五层拦截器全部未触发。

**根因：** BanInterceptor 靠关键词匹配（"好问题"、"虚构"、"跳过"），但"加载了skill文档但没执行workflow"没有匹配的关键词。这是语义级违规，不是词表级违规。

**本次漏拦清单：**
- "跑IF"→只读不跑（语义）| 未被拦
- "创建文件后声称记完了"→已有rule_060但无视（规则存在但执行者跳过）| 未被拦
- "orchestrator跳过clarify直接输出"→ClarifyInterceptor未触发（Agent没感知到"有选择"）| 未被拦

**防御策略：**
- 关键词层面无法解决——语义理解需要LLM，不是正则
- 靠B层哨兵（LLM语义判断，cmg-guard v1.1.0+）弥补——但哨兵在输出层扫描，同样不覆盖"中间步骤省略"
- **根本问题：CMG是输出层拦网，管不了Agent在执行层偷懒。这是架构级边界，不是bug。**
- 当前唯一有效手段：用户直接纠正→errors.jsonl→rule_060类规则→下次会话Guard提醒。但Guard不能替Agent执行。
