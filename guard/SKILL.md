...
**必须输出**: "Guard v4.5.1 已激活。读取 Canon {N} 条规则。效能分析: {X} 条规则正常 / {Y} 条降级 / {Z} 条过期。上下文感知: {重复检测/赶工检测/轻量模式} 就绪。动态清单: {论文类K项/代码类L项/通用类M项}。"
...
### 第三层：StepCompletenessInterceptor — 防跳步骤
...
4. 有未完成步骤 → 拦截: "⚠️ 检测到未完成步骤: {steps}。追加执行。"
5. 写入 intercept_log.jsonl
```

### 赶工模式联动（G3 · v4.5.0）

全局赶工模式激活时（启动检测到连续 ≥ 4 次 StepCompleteness 命中），该拦截器输出升级：
- 正常: "⚠️ 检测到未完成步骤: ..."
- 赶工: "🛑 赶工模式+跳步骤: {steps}。立刻补全，不得继续。"
