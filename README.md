...
| `!patterns` | 表格：规则 / 本会话命中 / 7天 / 置信度 / 该不该固化 |
...
```
  护栏线                              典则线                跨线共享
  ┌──────────────────┐          ┌──────────────┐    ┌────────────────────┐
  │ ralph-loop        │          │ plur          │    │ rtk-rewrite         │
  │ 跳步骤→闭环验证    │          │ 扩展规则来源   │    │ 压缩 token 60-90%   │
  │                    │          │               │    │                     │
  │ VBC                │          │ obsidian      │    │ hermes-agent-       │
  │ 声称完成→证据协议   │          │ rules/ 可视化  │    │ skill-authoring     │
  │                    │          │               │    │ 发布自检清单         │
  │ diagnose           │          └──────────────┘    └────────────────────┘
  │ 连续命中→根因诊断   │
