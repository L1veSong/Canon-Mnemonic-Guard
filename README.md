# 三省引擎 (CMG) — Canon-Mnemonic-Guard v5.3.2
...
| 线路 | 版本 | 定位 | 一句话 |
|------|------|------|--------|
| **典则线** Canon | v2.6.0 | 规则生产库 | 你说「记住」→ 写入规则 → 去重固化 |
| **护栏线** Guard | v4.7.1 | 规则执行器 + 闭环重试 | 五层拦截 → 风险分级 + 自动修正 |
| **忆存线** Mnemonic | v3.4.0 | 记忆状态层 | 数据源降级链 → 2次推草稿 |
| **外观层** CMG | v5.3.2 | 四包索引 | init 脚本 + SOUL 激活标记 + 推荐列表扫描 |

---

## v5.3.2 新特性：Guard 风险分级
...
```bash
python3 init.py
# 选 Y → SOUL.md 写入: [CMG v5.3.2] 加载 canon-mnemonic-guard 护栏规则
```
