...
_comment: "v5.3.2 同步Guard v4.7.1风险分级。Canon v2.6.0(规则分级+修正模板) + Guard v4.7.1(风险分级+闭环重试) + Mnemonic v3.4.0(2次推草稿)。"
...
| 版本 | 变更 |
|------|------|
| v5.3.2 | +同步Guard v4.7.1风险分级: hard规则按操作类型分流——不可逆暂停确认，可自动修复自动重试 |
...
**必须输出**: "三省引擎 v5.3.2 已激活。X 条禁止 / Y 条缺失 / Z 条偷懒。典则·护栏·忆存。四包制。"
...
📦 子包版本: ✅ 一致
   canon v2.6.0 / guard v4.7.1 / mnemonic v3.4.0
...
| 推荐 | 类型 | 增强点 | 验证 |
|------|------|--------|:--:|
| `rtk-rewrite` | Hermes 插件 | 压缩终端输出 60-90% token，gateway 自动挂钩 | ✅ |
| `plur` | MCP 服务器 | 扩展规则来源，CMG 扫盘读取 `~/.plur/engrams.yaml` | ✅ |
| `verification-before-completion` | Skill | Guard 拦截「声称完成但未验证」→ VBC 证据协议 | ✅ |
| `diagnose` | Skill | !diagnose 五阶段诊断 + 自动修复建议 | ✅ |
| `ralph-loop` | Skill | Guard 拦截跳步骤 → 多步骤闭环验证 | ✅ |

> 联动测试方法详见 `references/integration-test-patterns.md`

### 建议配套安装（独立使用，非 CMG 联动）

| 推荐 | 类型 | 用途 |
|------|------|------|
| `obsidian` | Skill | rules/*.md 可视化浏览 + 全文检索 |
| `karpathy-coding-guidelines` | Skill | 进攻型行为准则，与 Guard 理念互补 |
| `hermes-agent-skill-authoring` | Skill | 13 项发布自检清单 + 版本号 grep 验证 |────────────┐ │
...
四条优化建议经 DeepSeek 评审，详见 `references/v5.4.0-improvement-directions.md`：
...
**v5.4.0-alpha (当前):** 四包制(Canon v2.6.0/Guard v4.7.1/Mnemonic v3.4.0独立+CMG外观)
...
| M3 | CLI 命令实现 | hermes reflect 全为规格，依赖 Hermes 内核子命令支持 |
...
> **剩余 1 项：M3 CLI 命令。** 依赖 Hermes 内核支持子命令注册，非 CMG 独立可控。其余 16 项全部完成。
