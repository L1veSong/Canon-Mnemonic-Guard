...
> **定位：** 规则执行器。运行时读取 Canon 规则库 + Mnemonic 记忆库，独立执行 pre_action 前置拦截。**当前状态：拦截逻辑寄生在 Canon 线内。v4.0.0 起彻底剥离，独立为 Guard 线。**
...
v2.3   依赖解耦
v2.4   规则效果评分        v3.0   CLI 独立触发        v4.0   独立化拆分
  +角色制 producer         +角色制 memory             +角色制 guard
  system_anchor            background                 pre_action
                          v3.1   自动模式识别        v4.1   动态清单生成
                                                    v4.2   拦截效能分析
                                                    v4.3   上下文感知拦截
        ↓                       ↓                        ↓
        └───────────────────────┴────────────────────────┘
