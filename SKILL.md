...
_comment: "v5.1.0 四包制: Canon v2.4.1 + Guard v4.4.0 + Mnemonic v3.2.0 各自独立Skill包 + CMG外观索引"
...
**必须输出**: "三省引擎 v5.1.0 已激活。X 条禁止 / Y 条缺失 / Z 条偷懒。典则·护栏·忆存。四包制。"
...
**v5.1.0 (当前):** 四包制(Canon/Guard/Mnemonic独立+CMG外观) / 10条规则 / 7项优化完成
...
| G2 | 动态清单接入真实错误数据 | errors.jsonl 无运行时增量 |
| G3 | 上下文感知实现 | 计时器/计数器/升级逻辑全为规格 |
| G4 | 效能分析真实数据源 | 指标依赖真实拦截 |
...
| # | 方向 | 说明 |
|---|------|------|
| M1 | 模式识别数据源依赖 Guard | Guard 拦截器未运行→无增量 |
| M2 | ✅ 独立持久化（v3.2.0） | mnemonic_state.json |
| M3 | CLI 命令实现 | hermes reflect 全为规格 |
